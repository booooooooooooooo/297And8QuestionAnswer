...
\abstract{Question Answering (QA) is about making a computer program that could answer questions in natural language. It is one of the most challenging tasks in natural language processing. At the present, the state-of-art performance of QA is produced by applying neural network models. In this project, I build a QA system using an encoder-decode recurrent neural network architecture. Besides, I research on how to explain the model and how to reduce network size without decreasing accuracy. The baseline encoder-decoder recurrent neural network I apply is the match-lstm and answer pointer model. Based on it, I design three changes to explain some parts of the baseline model and try to reduce the size of the baseline model. My contributions include: I successfully implement match-lstm and answer pointer model by getting TODOADDRESULT; I compare the baseline model and three changes and find out some redundant design in the baseline model -  preprocessing layer and the $h_r$ in bidirecional match-lstm layer provides duplicate context information.
}
