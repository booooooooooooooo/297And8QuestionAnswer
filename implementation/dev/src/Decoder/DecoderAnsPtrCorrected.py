...
class DecoderAnsPtrCorrected(Decoder):
    ...
    def decode(self):
        ...
        with tf.variable_scope("decode_ans_ptr_corrected"):
            ...
            lstm_ans = tf.contrib.rnn.BasicLSTMCell(input_size)#state_size = input_size
            hidden_state = tf.zeros([batch_size, input_size])
            current_state = tf.zeros([batch_size, input_size])
            state = hidden_state, current_state
            #get beta_s
            F_s = tf.tanh( tf.matmul(self.H_r, tf.tile(tf.expand_dims(V, [0]), [batch_size, 1, 1]))
                        +  tf.expand_dims(tf.matmul(current_state, W_a) + b_a , [1]))#(batch_size, pass_max_length, num_units)
            beta_s = tf.reshape(tf.matmul(F_s,
                                          tf.tile(tf.reshape(v, [1, -1, 1]),
                                                  [batch_size, 1, 1])), (batch_size, -1))#(batch_size, pass_max_length)

            #lstm one step to updata
            prob_s = tf.nn.softmax(beta_s) * self.passage_mask#(batch_size, pass_max_length)
            lstm_input = tf.reshape(tf.matmul(tf.expand_dims(prob_s, [1]), self.H_r), (batch_size, input_size))#(batch_size, 2 * num_units)
            _, state = lstm_ans(lstm_input, state)
